...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("qwen/models/configs/v1_qwen2_5_7b_qlora_config.json"),
...
def run_post_training_benchmark(output_dir: Path) -> Path:
    command = [
        os.sys.executable,
        "qwen/scripts/evaluate_benchmark.py",
        "--model-path",
        str(output_dir),
    ]
    ...
    if report_path is None:
        report_path = Path("qwen/experiments") / f"benchmark_report-{output_dir.name}.json"
...
def refresh_training_summary() -> None:
    command = [
        os.sys.executable,
        "qwen/scripts/summarize_training_runs.py",
    ]
