...
DEFAULT_RUNS_DIR = Path("qwen/models/runs")
DEFAULT_REPORTS_DIR = Path("qwen/experiments")
DEFAULT_CSV_OUT = DEFAULT_REPORTS_DIR / "training_runs_summary.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Summarize Qwen training runs and their benchmark reports in a compact table."
    )
...
def write_csv(rows: list[dict], path: Path) -> Path:
    ...
    fieldnames = list(rows[0].keys()) if rows else [
        "run","run_date","run_time","status","epoch","global_step","train_examples",
        "validation_examples","train_runtime_s","train_steps_per_second","train_samples_per_second",
        "train_loss","benchmark_size","valid_json_rate","response_type_accuracy","report_path"
    ]
...
def render_table(rows: list[dict], csv_out: Path | None) -> str:
    headers = [
        ("run", "Run"),
        ("run_date", "Date"),
        ("run_time", "Time"),
    ...
    widths = {key: max(len(label), *(len(str(r[key])) for r in rendered_rows)) for key, label in headers}
    lines = ["  ".join(label.ljust(widths[key]) for key, label in headers)]
    lines.append("  ".join("-" * widths[key] for key, _ in headers))
    for rendered_row in rendered_rows:
        lines.append("  ".join(str(rendered_row[key]).ljust(widths[key]) for key, _ in headers))
